...
blank2 = []
blank = []
script = list(jsondata.values())

for a in script:
    blank.append(str(a))
    
for a in blank:
    blank2.append(a.upper())

scriptappended = ("".join(blank2))

scriptappendedlist = list(scriptappended)

listwithletters = []

for a,b in enumerate(scriptappendedlist):
    if b == "B" or b == "P" or b == "I":
        listwithletters.append(a)
        
for a in listwithletters:
    if scriptappendedlist[a+1] == ">":
        scriptappendedlist[a] = ""

unnecessary_punctuation = r"&$@[].,'#()-\"!?’_;:/"

for a in scriptappendedlist:
    if unnecessary_punctuation.find(a) >= 0 or a == ">" or a == "<" or a == "\n":
        scriptappendedlist.remove(a)
        

final = ("".join(scriptappendedlist))
...
final2 = list(final.split(" "))
count = []
word = []

for a in final2:
    if len(a) >= 3:
        count.append(final2.count(a))
        word.append(a)
...
for a in output:
    outputcount.append(output.count(a))
        
wordcount_dictionary = {k: v for k,v in enumerate(output)} #instead of enumerate k, replace all k with values from output.
...
r = 0
c = 0
while (1000*f + 600*r + 750*c )<= 18000:
    while (500*f + 750*r + 300*c )<= 12000:
        while (50*f + 45*r + 75*c )<= 1500:
            r+=1
            c+=1
            f+=1
...
Z = 10*(optimal_F) + 12*(optimal_R)+ 16*(optimal_C)
...
def Collatz(start_number):
    ...
    def Collatz(start_number):
    colattz_sequence = []
    collatz_length = len(colattz_sequence)
    #max_collatz = max(colattz_sequence)
    sequence_sum = sum(colattz_sequence)
    number = ""

    if start_number%2 == 0:
        number = start_number/2
        colattz_sequence.append(start_number)
        colattz_sequence.append(int(number))
    ...
    final_dictionary = {"colattz_sequence": colattz_sequence,
                        "collatz_length": len(colattz_sequence),
                        "max_collatz": max(colattz_sequence),
                        "sequence_sum": sum(colattz_sequence)}
...
def Collatz_winner(number_list):
    def Collatz(start_number):
        ...
        return(collatz_length,max_collatz)

    blank = []
    for a in number_list:
        blank.append(list(Collatz(a)))

    length = []
    maxval = []

    for a,b in enumerate(blank):
        length.append(blank[a][0])
        maxval.append(blank[a][1])

    lengthmax = max(length)
    maxmax = max(maxval)


    if length.index(lengthmax) == maxval.index(maxmax):
        return number_list[length.index(lengthmax)]
